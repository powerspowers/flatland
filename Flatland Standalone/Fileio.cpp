...
parse_level_tag(void)
{
	...
	// Get a pointer to the start of the map text.

	start_parsing_nested_tags();
	text = nested_text_to_string(TOKEN_LEVEL);
	stop_parsing_nested_tags();
...
load_config_file(void)
{
	...
	// First attempt to parse the configuration file in the new format.
	// Otherwise attempt to parse it in the old format.
...
destroy_spot_dir_list(spot_dir_entry *spot_dir_list)
{
	...
	while (spot_dir_list != NULL) {
		if (spot_dir_list->nested_spot_dir_list != NULL)
			destroy_spot_dir_list(spot_dir_list->nested_spot_dir_list);
		next_spot_dir_entry_ptr = spot_dir_list->next_spot_dir_entry_ptr;
		DEL(spot_dir_list, spot_dir_entry);
		spot_dir_list = next_spot_dir_entry_ptr;
	}
}

//------------------------------------------------------------------------------
// Add a new entry to the end of a spot directory list.
//------------------------------------------------------------------------------

static spot_dir_entry *
add_spot_dir_entry(spot_dir_entry *&spot_dir_list, 
				   spot_dir_entry *&last_spot_dir_entry_ptr)
{
	spot_dir_entry *spot_dir_entry_ptr;

	NEW(spot_dir_entry_ptr, spot_dir_entry);
	spot_dir_entry_ptr->nested_spot_dir_list = NULL;
	spot_dir_entry_ptr->next_spot_dir_entry_ptr = NULL;
	if (spot_dir_entry_ptr == NULL)
		return(NULL);
	if (last_spot_dir_entry_ptr != NULL)
		last_spot_dir_entry_ptr->next_spot_dir_entry_ptr = spot_dir_entry_ptr;
	else
		spot_dir_list = spot_dir_entry_ptr;
	last_spot_dir_entry_ptr = spot_dir_entry_ptr;
	return(spot_dir_entry_ptr);
}

//------------------------------------------------------------------------------
// Read the next line from a string, if one exists, removing all leading white
// space.
//------------------------------------------------------------------------------

static string
read_line(const char *&string_ptr)
{
	const char *line_ptr;
	string line;
	...
	if (*string_ptr != '\0') {
		...
		while (*string_ptr != '\n' && *string_ptr != '\0')
			string_ptr++;
		line.copy(line_ptr, string_ptr - line_ptr);
		if (*string_ptr == '\n')
			string_ptr++;
	}
	return(line);
...
//------------------------------------------------------------------------------
// Parse a spot directory list.
//------------------------------------------------------------------------------

static spot_dir_entry *
parse_spot_dir_list(int start_tag_token)
{
	spot_dir_entry *spot_dir_list, *last_spot_dir_entry_ptr;
	spot_dir_entry *spot_dir_entry_ptr;
	int tag_token;
	string spot_dir_entries, label, URL;
	char *line_ptr;

	spot_dir_list = NULL;
	last_spot_dir_entry_ptr = NULL;
	try {
		start_parsing_nested_tags();
		while (parse_next_nested_tag(start_tag_token, directory_tag_list, true,
			&tag_token))
			switch (tag_token) {
			case TOKEN_CATEGORY:
				if ((spot_dir_entry_ptr = add_spot_dir_entry(spot_dir_list, 
					last_spot_dir_entry_ptr)) == NULL)
					throw (char *)NULL;
				spot_dir_entry_ptr->label = category_name;
				spot_dir_entry_ptr->nested_spot_dir_list =
					parse_spot_dir_list(TOKEN_CATEGORY);
				break;
			case TOKEN_CHARACTER_DATA:
				spot_dir_entries = text_to_string();
				line_ptr = (char *)spot_dir_entries;
				while (*line_ptr != '\0') {
					label = read_line((const char *&)line_ptr);
					URL = read_line((const char *&)line_ptr);
					if (strlen(label) > 0 && strlen(URL) > 0 &&
						(spot_dir_entry_ptr = add_spot_dir_entry(spot_dir_list,
						last_spot_dir_entry_ptr)) != NULL) {
						spot_dir_entry_ptr->label = label;
						spot_dir_entry_ptr->URL = URL;
					}
				}
			}
		stop_parsing_nested_tags();
...
{
	// Open the directory file.  If this fails just return an empty list.

	spot_dir_list = NULL;
	if (!push_file(directory_file_path, directory_file_path, true))
		return;

	// Parse the directory file.

	try {
		parse_start_of_document(TOKEN_DIRECTORY, NULL, 0);
		parse_rest_of_document(true);
		spot_dir_list = parse_spot_dir_list(TOKEN_DIRECTORY);
	}

	// If the parsing failed, delete the partial spot directory list.
